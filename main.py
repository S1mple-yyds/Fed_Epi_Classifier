...
import pandas as pd
import numpy as np
from models.trainer import load_from_checkpoint, model_train ,model_whole, model_test, plot_client_metrics, plot_1_prediction
            ...
            for config['model'] in ['stgat','lstm']:
                for config['model_train'] in ['avg','prox']:
                    ...
                    #for i in [2**(x+1) for x in range(4)]:
                        for config['client_ratio'] in [3/6,2/6,1/6]:
                            for config['node_ratio'] in [0.6,0.5,0.4,0.3,0.2,0.1]:
                                config['N_CLIENTS'] = i
